...
Trains in ~5 minutes on a laptop CPU. Produces `models/baseline.pkl` which
...
Run (from repository root):
    python train/baseline.py       # writes models/baseline.pkl

Your job is to replace this file with something better. The grader only cares
about `predict.py` — this file just needs to produce a `baseline.pkl` that
`predict.py` can load.
"""
